...
train = read.csv("C:/Users/ivan/Google Drive/Machine learning/Kaggle/House Prices/train.csv", sep = "," , header = T) %>%
  select(sort(peek_vars())) %>% mutate(data = "train")
...
# PoolArea
train %>% ggplot(aes(SalePrice, fill = PoolArea)) + geom_histogram(bins = 50) + scale_x_continuous(labels = comma, n.breaks = 20)


# Utilities useless
train %>% select(Utilities) %>% na.omit() %>% group_by(Utilities) %>% summarise(Count = n())
full %<>% select(-Utilities)

# See most important numeric predicitores (Correlations with SalePrice)
...
# Change factors to characters to more easily tranforms data in integrers
full %<>% mutate_if(~ is.factor(.), as.character) %>% glimpse()
...
# LotFrontage (use median by neigborhood with package "simputation") / LotShape
...
full %<>%
  impute_median(LotFrontage ~ Neighborhood) %>%
